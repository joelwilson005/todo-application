...
import {fetchDeleteActionItem, fetchUpdateActionItem,} from "../data/todoListsSlice";
...
const ActionItem = ({actionItem, todoListId}) => {
    ...
    const [textDescription, setTextDescription] = useState(actionItem.textDescription);
    ...
    const handleTextDescriptionChange = (event) => {
        ...
        const payload = {
            textDescription: textDescription, completed: completed, id: actionItem.id, todoListId: todoListId,
        };
    ...
    const handleDeleteClick = (event) => {
        ...
        const payload = {
            id: actionItem.id, todoListId: todoListId,
        };
    ...
    return (isShown && (<AnimatePresence>
        <motion.div
            initial={{opacity: 0}}
            animate={{opacity: [0.25, 0.5, 0.75, 1]}}
            transition={{duration: 0.8}}
            exit={{
                scale: [1, 0.75, 0.5, 0.25, 0], transition: {duration: 0.4},
            }}
